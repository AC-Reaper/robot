...
class SimpleStanleyController(BaseController):
    """简化的Stanley控制器"""
    
    def __init__(self, k_crosstrack: float = 0.5, max_angular_velocity: float = 0.5):
        """
        初始化简化Stanley控制器
    ...
    def compute_control(self, robot_state: tuple, 
        ...
        # 获取目标点（稍微超前一点）
        target_idx = min(closest_idx + 3, len(reference_path) - 1)
        ...
        # 简化的Stanley控制律
        angular_velocity = self.k_crosstrack * heading_error
